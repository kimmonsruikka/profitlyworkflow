...
**Theoretical max attainable confidence under rules-v1 FV-v2 = 0.85** (sum of all six weights). **Practical max attainable today = 0.70.** The `underwriter_flagged` weight (0.15) cannot fire because parser-side underwriter resolution is missing — zero filings in the 90-day window prior to merge had any `underwriter_id` resolved (887 filings, 0 with `underwriter_id IS NOT NULL`). The signal is wired correctly on the scorer side and will activate when the parser gap is closed (tracked separately as a chore issue). The `_clamp_unit` floor still bounds output to `[0, 1]`. Calibration is empirical anyway — these weights are placeholders per `RulesV1Scorer` docstring — so the maximum-attainable being 0.70 instead of 1.00 doesn't change correctness, only theoretical headroom. Documented for honesty.

**Phase 1b note — confidence-distribution ceiling.** rules-v1's max confidence is **0.85 theoretical / 0.70 practical**, not 1.0. Do **not** normalize confidence to `[0, 1]` for cross-scorer comparison. Reason in raw weight space until the practical ceiling stabilizes (i.e., until the parser gap closes and dormant signals start firing). Cross-scorer comparison (e.g., rules-v1 vs the eventual GBDT) should treat each scorer's confidence on its own scale; the rescale lies about the distribution shape.
