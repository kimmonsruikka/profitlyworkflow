...
- **Celery tasks that touch the DB MUST use `task_local_session_factory()`.** The module-level `engine` / `AsyncSessionLocal` in `data/db.py` are bound to whichever event loop calls them first. Celery's prefork worker calls each task via `asyncio.run(...)` — fresh loop per task — so the cached engine produces `RuntimeError: got Future attached to a different loop`. Any session a Celery task opens must come from `task_local_session_factory()` (which creates a fresh engine on entry, disposes on exit) via the `session_from(factory)` helper. Module-level `get_session()` is fine for non-Celery code paths (FastAPI request handlers, Prefect flows) where the calling loop persists across the session lifetime.
